   ...
   while not current_level.is_empty():
      ...
      if current_part.prnt == 0:
         ...
         for per, qt in enumerate(need_lst, start=(1-neg)):
            i_per = per + neg -1
            if qt > 0:
               if 1 - (per-current_part.lt) > neg:
                  new_neg = 1 - (per-current_part.lt)
                  neg_dif = new_neg - neg
               ...
               if per > 0:
                  ...
                  if avail - need_lst[i_per] < current_part.ss:
                     short = need_lst[i_per] - avail
                     buy = (short // current_part.ls + (short % current_part.ls)) * current_part.ls
                  ...
                  when = per - current_part.lt
                  i_when = when + neg - 1
                  release_lst[i_when] = qt
                  for child, amount in current_part.bom.items():
                     child.mps[when] = amount*buy + child.mps[when]
            avail -= need_lst[i_per]
